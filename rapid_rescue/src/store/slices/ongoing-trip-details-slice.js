  ...
  reducers: {
    setOngoingTripDetails: (state, action) => {
      state.pickup_location =
        action.payload?.pickup_location ?? state.pickup_location;
      state.destination = action.payload?.destination ?? state.destination;
      // Ensure fare is always a number and handle null/undefined cases
      const newFare = action.payload?.fare;
      state.fare =
        typeof newFare === "number" && newFare >= 0 ? newFare : state.fare;
      ...
      state.driver_mobile =
        action.payload?.driver_mobile ?? state.driver_mobile;
...
export const { setOngoingTripDetails, clearTrip } =
  ongoingTripDetailsSlice.actions;
